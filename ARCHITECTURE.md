...
1. _Compiler:_ This runs in Node.js at component compilation time.
2. _Runtime:_ This runs in the browser at component runtime (or on the server in Node.js in the case of SSR).
...
The decision of whether to compile an HTML/JS file for CSR or SSR is made by a compiler flag. A file compiled with `@lwc/ssr-compiler` is intended to only be used with `@lwc/ssr-runtime`.
...
This architecture was created in the genesis of LWC SSR to facilitate development of the SSR system, and to ensure that it remains as close as possible to the CSR system.
...
These projects are typical "done" when implemented and rarely need to be touched. [`observable-membrane`](https://github.com/salesforce/observable-membrane) is also conceptually in this same group, even though it technically lives outside the LWC monorepo.
...
There are several internal packages for tests and performance benchmarking:
...
- _Virtual DOM._ Like Vue, LWC uses Virtual DOM even in cases where a component is authored in HTML. Unlike Vue, LWC does not have a mode where you can author VDOM directly (i.e. [`render()`](https://doc.vueframework.com/guide/render-function.html)).
- _DOM diffing._ Like Vue, LWC was originally authored with [`snabbdom`](https://github.com/snabbdom/snabbdom) but has since forked into its own implementation.
- _Proxies._ Like Vue v3, LWC uses the JavaScript [`Proxy`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Proxy) for reactivity.
...
This is an LWC-specific concept driven by the intense needs for backwards compatibility on the Salesforce Lightning platform. This is a whole topic on its own, but is largely covered by [the official Salesforce blog post](https://developer.salesforce.com/blogs/2024/01/introducing-component-level-api-versioning-for-lwc) on the topic.
