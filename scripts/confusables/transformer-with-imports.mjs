 ...
 * Now transforms imported identifiers with aliases: import { foo as ƒøø }
 */
export function transformSource(ast, source, analysis) {
    const replacements = [];
    const { publicIdentifiers } = analysis;

    // Track imports: original name -> confusable name
    const importedIdentifierMap = new Map();
    const parameterNames = new Set();
    const destructuredIdentifiers = new Set();
    const shorthandPropertyIdentifiers = new Set();

    traverse(ast, {
        // Collect all imported identifiers and prepare aliases
        ImportDeclaration(path) {
            path.node.specifiers.forEach((spec) => {
                if (spec.local && spec.local.type === 'Identifier') {
                    const originalName = spec.local.name;
                    const confusableName = transformIdentifier(originalName);
                    importedIdentifierMap.set(originalName, confusableName);
                }
            });
        },
    ...
    // Helper to check if an identifier should be skipped (not transformed at all)
    function shouldSkip(path, name) {
        // Skip if part of import declarations - those are handled separately
        if (
        ...
        // UNLESS it's an imported type that should use its alias
        if (isInTypeContext(path) && !importedIdentifierMap.has(name)) {
            return true;
        }

        // Skip public identifiers
        // UNLESS they're imported with an alias (in which case use the alias)
        if (publicIdentifiers.has(name) && !importedIdentifierMap.has(name)) {
            return true;
        }

        // Skip if part of export (but not import - we handle those specially)
        // UNLESS it's an imported identifier being re-exported (use the alias)
        if (path.parent?.type === 'ExportSpecifier' && !importedIdentifierMap.has(name)) {
            return true;
        }

        // Skip if it's a property key in object literal (might be public API)
        // This includes shorthand properties like { get, set }
        // Also includes destructuring keys like { path: modulePath }
        if (
    ...
    // Second pass: transform import specifiers to add aliases
    traverse(ast, {
        ImportSpecifier(path) {
            ...
            if (imported.type === 'Identifier' && local.type === 'Identifier') {
                const originalName = local.name;
                const confusableName = importedIdentifierMap.get(originalName);

                if (confusableName && confusableName !== originalName) {
                    if (imported.name === local.name) {
                        // Simple case: import { foo }
                        // Both names are the same, insert alias
                        // Insert " as ƒøø" after the identifier
                        replacements.push({
                            start: local.end,
                            end: local.end,
                            text: ` as ${confusableName}`,
                        });
                    } else {
                        // Already has alias: import { bar as foo }
        ...
        ImportDefaultSpecifier(path) {
            ...
            if (local.type === 'Identifier') {
                ...
                if (confusableName && confusableName !== originalName) {
                    // Transform: import foo from 'x' -> import ƒøø from 'x'
                    replacements.push({
                        start: local.start,
                        end: local.end,
                        text: confusableName,
        ...
        ImportNamespaceSpecifier(path) {
            ...
            if (local.type === 'Identifier') {
                ...
                if (confusableName && confusableName !== originalName) {
                    // Transform: import * as foo from 'x' -> import * as ƒøø from 'x'
                    replacements.push({
                        start: local.start,
                        end: local.end,
                        text: confusableName,
        ...
        Identifier(path) {
            ...
            // Check shouldSkip FIRST before any transformations
            if (shouldSkip(path, name)) {
                return;
            }

            // Check if this is an imported identifier being used
            if (importedIdentifierMap.has(name)) {
                // Skip if it's a property key in destructuring { path: x }
                // In this case, 'path' is matching the object's property name, not using the import
                if (
                    path.parent?.type === 'ObjectProperty' &&
                    path.parent.key === path.node &&
                    !path.parent.computed
                ...
                // Use the confusable version
                const confusableName = importedIdentifierMap.get(name);
                if (
                    confusableName &&
                    confusableName !== name &&
