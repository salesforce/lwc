...
if (packageName === '@lwc/synthetic-shadow') {
    // Here we wrap all of synthetic shadow in a check for lwcRuntimeFlags.DISABLE_SYNTHETIC_SHADOW, so that
    // synthetic shadow is not loaded if the flag is set.
    ...
    if (!lwcRuntimeFlags.DISABLE_SYNTHETIC_SHADOW) {
    `
        .replaceAll(/\n {4}/g, '\n')
        .trimEnd();
    footer += '\n}';
