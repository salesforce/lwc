...
const ATTRS_CUSTOM_ELEMENT_TRANFORMS = {
    accesskey: 'accessKey',
    tabindex: 'tabIndex',
    'class': 'className',
};
...
export function isReflective(tagName: string, attrName: string) {
    return !isCustomAttribute(attrName);

}

export function getPropertyNameFromAttrName(attrName: string, tagName: string) {
    const normalize = isCustomElement(tagName) ? ATTRS_CUSTOM_ELEMENT_TRANFORMS[attrName] : ATTRS_TRANFORMS[attrName];
    return normalize || ((isCustomAttribute(attrName) || isSVG(tagName)) ? attrName : toCamelCase(attrName));
}
