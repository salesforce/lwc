...
it('should throw an error when the passing a non iterable', () => {
    ...
    expect(() => document.body.appendChild(elm)).toThrowError(
        /Invalid template iteration for value `\[object Object\]` in \[object:vm undefined \(\d+\)\]. It must be an array-like object and not `null` nor `undefined`.|Invalid template iteration for value `\[object ProxyObject\]` in \[object:vm undefined \(\d+\)\]. It must be an array-like object and not `null` nor `undefined`./
    );
});
