...
import LwcManual from 'x/lwcManual';
...
describe('Element.querySelector* shadow DOM scoping', () => {
    ...
    it('should return the right Elements when using lwc:mode=manual', () => {
        const elm = createElement('x-lwc-manual', { is: LwcManual });
        document.body.appendChild(elm);

        const container = elm.shadowRoot.firstChild;
        ...
        expect(elm.querySelector('.outer')).toBe(null);
        expect(elm.shadowRoot.querySelector('.outer')).toBe(outerDiv);
        expect(container.querySelector('.outer')).toBe(outerDiv);

        expect(elm.querySelector('.inner')).toBe(null);
        expect(elm.shadowRoot.querySelector('.inner')).toBe(innerDiv);
        expect(container.querySelector('.inner')).toBe(innerDiv);
        expect(outerDiv.querySelector('.inner')).toBe(innerDiv);
    });
});
