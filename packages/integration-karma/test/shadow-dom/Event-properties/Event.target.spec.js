...
import XParentWithDomManualChild from 'x/parentWithDomManualChild';
...
describe('When event target is accessed async', () => {
    it('#W-6586380 - should return the original target, if original target node is not keyed', done => {
        const elm = createElement('x-parent-with-dom-manual-child', {
            is: XParentWithDomManualChild,
        });
        document.body.appendChild(elm);
        const child = elm.shadowRoot.querySelector('x-child-with-lwc-dom-manual');
