...
function defaultCompareFn(vnode1: VNode, vnode2: VNode): boolean {
    return (
        vnode1.nt === vnode2.nt &&
...
function isTextVNode(vnode: VNode): vnode is VText {
    return vnode.nt === TEXT_NODE;
}

function isCommentVNode(vnode: VNode): vnode is VComment {
    return vnode.nt === COMMENT_NODE;
}
...
const hooks: Array<keyof LifeCycleHooks> = [
    'create',
    'update',
    'remove',
    'destroy',
...
export function init(
    ...
    api: DOMAPI,
    compareFn?: (vnode1: VNode, vnode2: VNode) => boolean,
): PatchFunction {
    ...
    function createElm(vnode: VNode, insertedVnodeQueue: VNodeQueue): Node {
        ...
        if (isElementVNode(vnode)) {
            ...
            const elm = (vnode.elm = isDef((i = data.ns))
                ? api.createElementNS(i, tag, data.uid, data.lightDom)
                : api.createElement(tag, data.uid, data.lightDom));
    ...
    const patch = function patch(oldVnode, vnode) {
        ...
        for (i = 0, n = insertedVnodeQueue.length; i < n; ++i) {
            ...
            ((insertedVnodeQueue[i].data.hook as Hooks).insert as InsertHook)(
