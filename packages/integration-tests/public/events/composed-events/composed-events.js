(function (lwc) {
  ...
  function tmpl($api, $cmp, $slotset, $ctx) {
    ...
    return [api_element("button", {
      ...
      key: 2,
      on: {
        "click": _m0 || __setKey($ctx, "_m0", api_bind($cmp._ES5ProxyType ? $cmp.get("handleEventClick") : $cmp.handleEventClick))
      }
    }, [api_text("Dispatch composed Event")]), api_element("button", {
      classMap: {
        "composed-custom-event": true
      },
      key: 3,
      on: {
        "click": _m1 || __setKey($ctx, "_m1", api_bind($cmp._ES5ProxyType ? $cmp.get("handleCustomEventClick") : $cmp.handleCustomEventClick))
      }
    }, [api_text("Dispatch composed CustomEvent")])];
  }
  ...
  function (_LightningElement) {
    ...
    _createClass(Child, [{
      ...
      value: function handleEventClick(event) {
        __callKey1(event._ES5ProxyType ? event.get("target") : event.target, "dispatchEvent", new Event('event', {
          bubbles: true,
          composed: true
        }));
      }
    }, {
      key: "handleCustomEventClick",
      value: function handleCustomEventClick(event) {
        __callKey1(event._ES5ProxyType ? event.get("target") : event.target, "dispatchEvent", new CustomEvent('customevent', {
          bubbles: true,
          composed: true
        }));
      }
  ...
  __setKey(tmpl$1, "stylesheetTokens", {
    hostAttribute: "integration-composed-events_composed-events-host",
    shadowAttribute: "integration-composed-events_composed-events"
  });
  ...
  var element = lwc.createElement('integration-composed-events', {
    is: Cmp,
    fallback: fallback
  });
