...
// TODO [#0]: Move style injection routing to renderer. We need a different approach to inject the
// styles when running in the DOM or on the server
export function evaluateCSS(
    _stylesheets: StylesheetFactory[],
    _hostAttribute: string,
    _shadowAttribute: string
): VNode | null {
    return null;
