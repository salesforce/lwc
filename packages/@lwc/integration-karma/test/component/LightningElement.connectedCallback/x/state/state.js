  ...
  const stateDefinition = (...args) => {
    class StateManagerSignal extends SignalBaseClass {
      constructor() {
        ...
        this.internalStateShape = defineStateCallback(atom, computed, update, fromContext2)(...args);
      ...
      computeValue() {
        const computedValue = Object.fromEntries(
          Object.entries(this.internalStateShape).filter(([, signalOrUpdater]) => signalOrUpdater).map(([key, signalOrUpdater]) => {
...
const consumeStateFactory = defineState((atom, computed, update, fromContext) => (initialName = 'bar') => {
    const name = atom(initialName);
