...
const { GITHUB_RUN_ID, IS_CI, SAUCE_ACCESS_KEY, SAUCE_TUNNEL_ID, SAUCE_USERNAME } = options;
function getSauceSection({ suiteName, customData }: { suiteName: any; customData: any }) {
...
function createPattern(location: any, config = {}) {
    return {
        ...config,
        pattern: location,
    };
...
function getSauceConfig(
    config: { reporters: any; plugins: any },
    { suiteName, customData, browsers }: any
) {
    return {
        sauceLabs: getSauceSection({ suiteName, customData }),

        browsers: browsers.map((browser: { label: any }) => browser.label),
        customLaunchers: browsers.reduce(
            (
                acc: any,
                browser: {
                    label: any;
