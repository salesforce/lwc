...
export default function lwc(options: RollupLwcOptions = {}): Plugin {
    let { rootDir, modules = [] } = options;
    const { stylesheetConfig, sourcemap = false } = options;
    ...
    return {
        ...
        buildStart({ input }) {
            ...
            modules = [...modules, { dir: rootDir }, ...DEFAULT_LWC_MODULES];
        },
