...
const EXPECTED_HTML_FILENAME_SYNTH = 'expected-synth.html';
const EXPECTED_CSS_FILENAME_SYNTH = 'expected-synth.css';
    ...
    for (const synth of [false,true]) {
        for (const caseName of fixtures) {
            ...
            testFn(`${caseName}`, async () => {
                ...
                const actual = result.html;
                const actualCss = result.styles ? result.styles.join('\n') : '';
