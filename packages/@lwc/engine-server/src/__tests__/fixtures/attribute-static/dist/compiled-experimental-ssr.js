...
async function* tmpl(props, attrs, slotted, Cmp, instance, stylesheets) {
  ...
  yield "<div class=\"foo bar foo-bar\" style=\"color: red; background-color: blue\" data-foo=\"foo\"></div>";
  if (Cmp.renderMode !== 'light') {
    yield '</template>';
  }
}
...
async function* generateMarkup(tagName, props, attrs, slotted) {
  ...
  yield* tmplFn(props, attrs, slotted, AttributeStatic, instance, defaultStylesheets);
  yield `</${tagName}>`;
}
