...
const stylesheetScopeToken = "lwc-ut980mjsd1";
...
async function* tmpl(props, attrs, slotted, Cmp, instance) {
  ...
  yield "<!-- This is an HTML comment --><button";
  yield stylesheetScopeTokenClass;
  yield ">click me</button>";
  if (Cmp.renderMode !== 'light') {
    yield '</template>';
  }
}
...
async function* generateMarkup(tagName, props, attrs, slotted) {
  ...
  yield '>';
  yield* tmplFn(props, attrs, slotted, CommentsBasic, instance);
  yield `</${tagName}>`;
}
