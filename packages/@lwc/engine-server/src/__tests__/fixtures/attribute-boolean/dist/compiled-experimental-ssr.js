...
const stylesheetScopeToken = "lwc-58mfn9bdhrf";
...
async function* tmpl(props, attrs, slotted, Cmp, instance) {
  ...
  yield " required readonly checked>";
  if (Cmp.renderMode !== 'light') {
    yield '</template>';
  }
}
...
class AttributeBoolean extends LightningElement {}
const __REFLECTED_PROPS__ = [];
async function* generateMarkup(tagName, props, attrs, slotted) {
  attrs = attrs ?? ({});
  const instance = new AttributeBoolean({
  ...
  yield '>';
  yield* tmplFn(props, attrs, slotted, AttributeBoolean, instance);
  yield `</${tagName}>`;
}

const tagName = 'x-attribute-boolean';

export { AttributeBoolean as default, generateMarkup, tagName };
