...
it('should transform spellcheck to boolean false when attr is "false"', () => {
    const { calls, apis } = mockRenderApi();
    ...
    expect(calls).toMatchObject([
        [
            'api_component#1',
            ['x-foo', XFoo, { key: expect.any(Number), props: { spellcheck: false } }, []],
        ],
    ]);
});

it('should transform spellcheck to boolean when attr is "truthy"', () => {
    const { calls, apis } = mockRenderApi();
    ...
    expect(calls).toMatchObject([
        [
            'api_component#1',
            ['x-foo', XFoo, { key: expect.any(Number), props: { spellcheck: true } }, []],
        ],
    ]);
});

it('should not transform spellcheck to boolean when attr is an expression', () => {
    const { calls, apis } = mockRenderApi();
    const expectedSpellcheckValue = 'expected-spellcheck-value';
    const $cmp = { computedSpellcheck: expectedSpellcheckValue };
    ...
    const renderFn = compileToFunction(`
        <template>
            <x-foo spellcheck={computedSpellcheck}></x-foo>
        </template>
    `);

    renderFn(modules)(apis, $cmp);
    ...
    expect(calls).toMatchObject([
        [
            ...
            [
                'x-foo',
                XFoo,
                { key: expect.any(Number), props: { spellcheck: expectedSpellcheckValue } },
                [],
            ],
        ],
    ]);
