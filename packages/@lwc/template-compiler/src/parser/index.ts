...
function parseElement(ctx: ParserCtx, elementNode: parse5.Element, parent?: IRElement): IRElement {
    ...
    applyForEach(ctx, element);
    applyIterator(ctx, element);
    applyIf(ctx, element);
    applyHandlers(ctx, element);
    applyComponent(ctx, element);
    applySlot(ctx, element);
    applyKey(ctx, element);
    applyLwcDirectives(ctx, element);
    applyAttributes(ctx, element);
    validateElement(ctx, element);
    validateAttributes(ctx, element);
    validateProperties(ctx, element);

    parseChildren(ctx, element);
    validateChildren(ctx, element);

    return element;
}

function parseChildren(ctx: ParserCtx, parent: IRElement): void {
...
function parseText(ctx: ParserCtx, node: parse5.TextNode, parent: IRElement): IRText[] {
    ...
    for (const token of tokenizedContent) {
        ...
        if (isExpression(token)) {
            try {
                value = parseExpression(ctx, token);
            } catch (error) {
                ctx.addDiagnostic(
                    normalizeToDiagnostic(
...
function parseComment(ctx: ParserCtx, node: parse5.CommentNode, parent: IRElement): IRComment {
...
function getTemplateRoot(
    ...
    if (validRoots.length > 1) {
        ...
        ctx.warnAtLocation(ParserDiagnostics.MULTIPLE_ROOTS_FOUND, [], duplicateRoot);
    }

    const [root] = validRoots;

    if (!root || !parse5Utils.isElementNode(root)) {
        ctx.warnAtLocation(ParserDiagnostics.MISSING_ROOT_TEMPLATE_TAG);
    } else {
        return root;
    }
}

function applyHandlers(ctx: ParserCtx, element: IRElement) {
    let eventHandlerAttribute = getTemplateAttribute(ctx, element, EVENT_HANDLER_RE);
    while (eventHandlerAttribute) {
        ...
        if (!eventName.match(EVENT_HANDLER_NAME_RE)) {
            return ctx.warnOnIRNode(ParserDiagnostics.INVALID_EVENT_NAME, eventHandlerAttribute, [
                eventName,
            ]);
        }
        ...
        eventHandlerAttribute = getTemplateAttribute(ctx, element, EVENT_HANDLER_RE);
    }
}

function applyIf(ctx: ParserCtx, element: IRElement) {
    const ifAttribute = getTemplateAttribute(ctx, element, IF_RE);
    if (ifAttribute) {
        ...
        if (!VALID_IF_MODIFIER.has(modifier)) {
            return ctx.warnOnIRNode(ParserDiagnostics.UNEXPECTED_IF_MODIFIER, ifAttribute, [
                modifier,
            ]);
        }
...
function applyLwcDirectives(ctx: ParserCtx, element: IRElement) {
    const lwcAttribute = getTemplateAttribute(ctx, element, LWC_RE);
    if (!lwcAttribute) {
        return;
    }
    ...
    if (
        ...
        return ctx.warnOnIRNode(ParserDiagnostics.UNKNOWN_LWC_DIRECTIVE, element, [
            lwcAttribute.name,
            `<${element.tag}>`,
        ]);
    }
    ...
    const lwcOpts = {};
    applyLwcDynamicDirective(ctx, element, lwcOpts);
    applyLwcDomDirective(ctx, element, lwcOpts);
    applyLwcRenderModeDirective(ctx, element, lwcOpts);
    applyLwcPreserveCommentsDirective(ctx, element, lwcOpts);

    element.lwc = lwcOpts;
}

function applyLwcRenderModeDirective(ctx: ParserCtx, element: IRElement, lwcOpts: LWCDirectives) {
    ...
    if (
        ...
        return ctx.warnOnIRNode(ParserDiagnostics.LWC_RENDER_MODE_INVALID_VALUE, element);
    }
...
function applyLwcDynamicDirective(ctx: ParserCtx, element: IRElement, lwcOpts: LWCDirectives) {
    const lwcDynamicAttribute = getTemplateAttribute(ctx, element, LWC_DIRECTIVES.DYNAMIC);
    ...
    if (!ctx.config.experimentalDynamicDirective) {
        return ctx.warnOnIRNode(ParserDiagnostics.INVALID_OPTS_LWC_DYNAMIC, element, [
            `<${element.tag}>`,
        ]);
    }
    ...
    if (!isCustomElement(element)) {
        return ctx.warnOnIRNode(ParserDiagnostics.INVALID_LWC_DYNAMIC_ON_NATIVE_ELEMENT, element, [
            `<${element.tag}>`,
        ]);
    }

    if (lwcDynamicAttribute.type !== IRAttributeType.Expression) {
        return ctx.warnOnIRNode(ParserDiagnostics.INVALID_LWC_DYNAMIC_LITERAL_PROP, element, [
            `<${element.tag}>`,
        ]);
    }
...
function applyLwcDomDirective(ctx: ParserCtx, element: IRElement, lwcOpts: LWCDirectives) {
    const lwcDomAttribute = getTemplateAttribute(ctx, element, LWC_DIRECTIVES.DOM);
    ...
    if (getRenderMode(element) === LWCDirectiveRenderMode.light) {
        return ctx.warnOnIRNode(ParserDiagnostics.LWC_DOM_INVALID_IN_LIGHT_DOM, element, [
            `<${element.tag}>`,
        ]);
    }

    if (isCustomElement(element)) {
        return ctx.warnOnIRNode(ParserDiagnostics.LWC_DOM_INVALID_CUSTOM_ELEMENT, element, [
            `<${element.tag}>`,
        ]);
    }

    if (element.tag === 'slot') {
        return ctx.warnOnIRNode(ParserDiagnostics.LWC_DOM_INVALID_SLOT_ELEMENT, element);
    }
    ...
    if (
        ...
        const possibleValues = Object.keys(LWCDirectiveDomMode)
            .map((value) => `"${value}"`)
            .join(', or ');
        return ctx.warnOnIRNode(ParserDiagnostics.LWC_DOM_INVALID_VALUE, element, [possibleValues]);
    }

    lwcOpts.dom = lwcDomAttribute.value as LWCDirectiveDomMode;
}

function applyForEach(ctx: ParserCtx, element: IRElement) {
    const forEachAttribute = getTemplateAttribute(ctx, element, 'for:each');
    const forItemAttribute = getTemplateAttribute(ctx, element, 'for:item');
    const forIndex = getTemplateAttribute(ctx, element, 'for:index');
...
function applyIterator(ctx: ParserCtx, element: IRElement) {
    const iteratorExpression = getTemplateAttribute(ctx, element, ITERATOR_RE);
    ...
    if (element.forEach) {
        return ctx.warnOnIRNode(ParserDiagnostics.INVALID_FOR_EACH_WITH_ITERATOR, element, [
...
function applyKey(ctx: ParserCtx, element: IRElement) {
    const keyAttribute = getTemplateAttribute(ctx, element, 'key');
    if (keyAttribute) {
        ...
        return ctx.warnOnIRNode(ParserDiagnostics.MISSING_KEY_IN_ITERATOR, element, [element.tag]);
    }
}

function applyComponent(ctx: ParserCtx, element: IRElement) {
    const { tag } = element;
...
function applySlot(ctx: ParserCtx, element: IRElement) {
    ...
    if (element.forEach || element.forOf || element.if) {
        return ctx.warnOnIRNode(ParserDiagnostics.SLOT_TAG_CANNOT_HAVE_DIRECTIVES, element);
    }

    // Can't handle slots in applySlot because it would be too late for class and style attrs
    if (getRenderMode(element) === LWCDirectiveRenderMode.light) {
        const invalidAttrs = attrs.filter(({ name }) => name !== 'name').map(({ name }) => name);

        if (invalidAttrs.length > 0) {
            return ctx.warnOnIRNode(ParserDiagnostics.LWC_LIGHT_SLOT_INVALID_ATTRIBUTES, element, [
                invalidAttrs.join(','),
            ]);
        }
    }
    ...
    const nameAttribute = getTemplateAttribute(ctx, element, 'name');
    if (nameAttribute) {
...
function applyAttributes(ctx: ParserCtx, element: IRElement) {
    const { tag, attrsList: attrs } = element;

    attrs.forEach((rawAttr) => {
        const attr = getTemplateAttribute(ctx, element, attributeName(rawAttr));
        if (!attr) {
            return;
        }
        ...
        if (attr.type === IRAttributeType.String) {
            if (name === 'id') {
                ...
                if (/\s+/.test(value)) {
                    ctx.warnOnIRNode(ParserDiagnostics.INVALID_ID_ATTRIBUTE, attr, [value]);
                }

                if (isInIteration(element)) {
                    ctx.warnOnIRNode(ParserDiagnostics.INVALID_STATIC_ID_IN_ITERATION, attr, [
                        value,
                    ]);
                }

                if (ctx.seenIds.has(value)) {
                    ctx.warnOnIRNode(ParserDiagnostics.DUPLICATE_ID_FOUND, attr, [value]);
                } else {
...
function validateElement(ctx: ParserCtx, element: IRElement) {
    const { tag, namespace, parent, location, __original: node } = element;
    const isRoot = !parent;

    if (isRoot) {
        if (tag !== 'template') {
            return ctx.warnOnIRNode(ParserDiagnostics.ROOT_TAG_SHOULD_BE_TEMPLATE, element, [tag]);
        }
    ...
    if (!isVoidElement && !hasClosingTag && tag === tag.toLocaleLowerCase()) {
        ctx.warnOnIRNode(ParserDiagnostics.NO_MATCHING_CLOSING_TAGS, element, [element.tag]);
    }

    if (tag === 'style' && namespace === HTML_NAMESPACE_URI) {
        ctx.warnOnIRNode(ParserDiagnostics.STYLE_TAG_NOT_ALLOWED_IN_TEMPLATE, element);
        ...
        if (!isRoot && !hasAttributes) {
            ctx.warnOnIRNode(ParserDiagnostics.NO_DIRECTIVE_FOUND_ON_TEMPLATE, element);
        }
    } else {
        const isNotAllowedHtmlTag = DISALLOWED_HTML_TAGS.has(tag);
        if (namespace === HTML_NAMESPACE_URI && isNotAllowedHtmlTag) {
            return ctx.warnOnIRNode(ParserDiagnostics.FORBIDDEN_TAG_ON_TEMPLATE, element, [tag]);
        }
        ...
        if (!isKnownTag) {
            return ctx.warnOnIRNode(ParserDiagnostics.UNKNOWN_HTML_TAG_IN_TEMPLATE, element, [tag]);
        }
    }
}
...
function validateChildren(ctx: ParserCtx, element: IRElement) {
    ...
    if (element.lwc?.dom && effectiveChildren.length > 0) {
        return ctx.warnOnIRNode(ParserDiagnostics.LWC_DOM_INVALID_CONTENTS, element);
    }
}

function validateAttributes(ctx: ParserCtx, element: IRElement) {
    const { tag, attrsList: attrs } = element;
    ...
    attrs.forEach((attr) => {
        ...
        if (isProhibitedIsAttribute(attrName)) {
            ctx.warnOnIRNode(ParserDiagnostics.IS_ATTRIBUTE_NOT_SUPPORTED, element, [
                attrName,
                tag,
            ]);
        }
        ...
        if (isTabIndexAttribute(attrName)) {
            if (!isExpression(attr.value) && !isValidTabIndexAttributeValue(attr.value)) {
                ctx.warnOnIRNode(ParserDiagnostics.INVALID_TABINDEX_ATTRIBUTE, element);
            }
        }
        ...
        if (tag === 'iframe' && attrName === 'srcdoc') {
            ctx.warnOnIRNode(ParserDiagnostics.FORBIDDEN_IFRAME_SRCDOC_ATTRIBUTE, element);
        }
    });
}
...
function getPreserveComments(ctx: ParserCtx, element: IRElement): boolean {
    return getRoot(element).lwc?.preserveComments?.value ?? ctx.config.preserveHtmlComments;
}
