...
import { ParserDiagnostics, captureDiagnostic } from '@lwc/errors';
...
export function normalizeAttributeValue(
    ...
    if (isBooleanAttribute(name)) {
        if (value === 'true') {
            throw captureDiagnostic(ParserDiagnostics.BOOLEAN_ATTRIBUTE_TRUE(tag, name, value));
        } else if (value === 'false') {
            throw captureDiagnostic(ParserDiagnostics.BOOLEAN_ATTRIBUTE_FALSE(tag, name, value));
        }
    }
    ...
    if (!isEscaped && isExpression(value)) {
        if (isQuoted) {
            ...
            throw captureDiagnostic(
                ParserDiagnostics.AMBIGUOUS_ATTRIBUTE_VALUE(raw, unquoted, escaped)
            );
        }
        ...
        throw captureDiagnostic(ParserDiagnostics.AMBIGUOUS_ATTRIBUTE_VALUE_STRING(raw, escaped));
    }
