...
import DynamicTemplate, { template1, template2 } from 'c/dynamicTemplate';
import RenderThrow from 'c/renderThrow';
import RenderInvalid from 'c/renderInvalid';
...
function testInvalidTemplate(type, template) {
    it(`throws an error if returns ${type}`, () => {
        const elm = createElement('c-dynamic-template', { is: DynamicTemplate });
...
it('should associate the component stack when the invocation throws', () => {
    const elm = createElement('c-render-throw', { is: RenderThrow });
    ...
    expect(error.wcStack).toBe('<c-render-throw>');
});
...
it('throws an error when render() returns an invalid value', () => {
    const elm = createElement('c-render-invalid', { is: RenderInvalid });
    expect(() => document.body.appendChild(elm)).toThrowCallbackReactionError(
        /Invalid template returned by the render\(\) method on c-render-invalid/
    );
});
