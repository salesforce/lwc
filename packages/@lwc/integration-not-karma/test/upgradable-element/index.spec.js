...
import InteropParent from 'c/interopParent';
import Interop from 'c/interop';
import Child from 'c/child';
import DuplicateChild from 'c/dupChild';

// Regression test for issue W-8059125
it('element in template does not reset element created by createElement api', () => {
    // First time the c-interop tag will be registered
    const interop = createElement('c-interop', { is: Interop });
    document.body.appendChild(interop);

    expect(interop.shadowRoot.querySelector('.content')).not.toBeNull();

    // Second time the c-interop tag will be processed in the parent's template
    const interopParent = createElement('c-interop-parent', { is: InteropParent });
...
it('element created by createElement api does not reset element created in template', () => {
    // First time c-interop is created in c-interop-parent's template
    const interopParent = createElement('c-interop-parent', { is: InteropParent });
    document.body.appendChild(interopParent);

    // Second time c-interop is created using createElement() api
    const interop = createElement('c-interop', { is: Interop });
    document.body.appendChild(interop);

    const div = interopParent.shadowRoot
        .querySelector('c-interop')
...
it('should create elements with correct component behavior even when they share same tag name', () => {
    const childElm = createElement('c-child', { is: Child });
    document.body.appendChild(childElm);
    expect(childElm.tagName.toLowerCase()).toBe('c-child');
    ...
    const anotherChildElm = createElement('c-child', { is: DuplicateChild });
    document.body.appendChild(anotherChildElm);
    expect(anotherChildElm.tagName.toLowerCase()).toBe('c-child');
